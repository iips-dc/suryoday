...
function check_baithak($baithak_id,$user_id,$donation_id){
	...
	$query = "SELECT * FROM `visit_details` WHERE `user_id` = '".$user_id."' AND `baithak_id` = '".$baithak_id."' ";	
	$query_run = mysqli_query($con , $query);
	$num_rows = mysqli_num_rows($query_run);
	if($num_rows == 1){
		$query = "UPDATE `visit_details` SET `donation_id`= '".$donation_id."' WHERE `user_id` = '".$user_id."' AND `baithak_id` = '".$baithak_id."'";
		if($query_run = mysqli_query($con,$query)){
			echo 'Donation is made in baithak<br>';
			}
		}
	else{
		echo "Baithak id or user id was not found.<br>";
		}
	mysqli_close($con);
	}
...
if(isset($_POST['DonationSubmit'])){//When submit button is clicked
	...
	if(!empty($_POST['donationType'])&&!empty($_POST['receivedBy'])&&!empty($_POST['for_project'])){
		$for_project = mysqli_real_escape_string($con,htmlentities($_POST['for_project']));	
		...
		$date = date("Y-m-d");
			if(($received_by == -1)){//if $recieved by are not equal to -1
				echo "Please select valid entries from recieved by";
				echo '<br><a href="index.php">Back to donations</a>';
			}
			else{
				if($for_project == 'yes'){
